...
from nbs.bluetopo.cli.cli import str_to_bool
...
class TestBuildVrtCommand:
    def test_parses_all_args(self):
        from nbs.bluetopo.cli.cli import build_vrt_command
        with mock.patch("sys.argv", ["build_vrt", "-d", "/tmp/test", "-s", "bluetopo", "-r", "true"]):
            with mock.patch("nbs.bluetopo.cli.cli.build_vrt") as mock_bv:
                build_vrt_command()
...
class TestFetchTilesCommand:
    def test_parses_all_args(self):
        from nbs.bluetopo.cli.cli import fetch_tiles_command
        with mock.patch("sys.argv", ["fetch_tiles", "-d", "/tmp/test", "-g", "polygon.shp", "-s", "bag", "-u"]):
            with mock.patch("nbs.bluetopo.cli.cli.fetch_tiles") as mock_ft:
                fetch_tiles_command()
                mock_ft.assert_called_once_with(
                    project_dir="/tmp/test",
                    desired_area_filename="polygon.shp",
                    untrack_missing=True,
                    data_source="bag",
                )

    def test_default_no_untrack(self):
        from nbs.bluetopo.cli.cli import fetch_tiles_command
        with mock.patch("sys.argv", ["fetch_tiles", "-d", "/tmp/test"]):
            with mock.patch("nbs.bluetopo.cli.cli.fetch_tiles") as mock_ft:
                fetch_tiles_command()
                args = mock_ft.call_args
                assert args.kwargs["untrack_missing"] is False
...
class TestUntrackFlag:
    def test_untrack_long_form(self):
        from nbs.bluetopo.cli.cli import fetch_tiles_command
        with mock.patch("sys.argv", ["fetch_tiles", "-d", "/tmp/test", "--untrack"]):
            with mock.patch("nbs.bluetopo.cli.cli.fetch_tiles") as mock_ft:
                fetch_tiles_command()
                assert mock_ft.call_args.kwargs["untrack_missing"] is True

    def test_untrack_short_form(self):
        from nbs.bluetopo.cli.cli import fetch_tiles_command
        with mock.patch("sys.argv", ["fetch_tiles", "-d", "/tmp/test", "-u"]):
            with mock.patch("nbs.bluetopo.cli.cli.fetch_tiles") as mock_ft:
                fetch_tiles_command()
                assert mock_ft.call_args.kwargs["untrack_missing"] is True
